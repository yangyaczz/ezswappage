    ...
    <section className="grid gap-0 md:gap-4 grid-cols-1 grid-rows-[65px,auto] sm:grid-rows-[65px,auto] sm:grid-cols-[90px,auto] md:grid-rows-1">
      ...
      <div className="flex flex-col items-start justify-start gap-y-4">
        <header className="flex flex-wrap items-baseline justify-start max-w-lg min-w-full gap-0 leading-4 sm:justify-end">
          <p className="flex items-baseline mx-1 font-bold justify-self-start grow text-md md:text-lg lg:text-2xl whitespace-nowrap">
            {name}
            ...
            <p className="mx-1 text-sm align-baseline md:mx-3">
              {languageModel.Vol.toUpperCase()}: {totalVolume}{" "}
              {tradingCurrencyName}
            </p>

            <div className="flex items-center justify-start mx-1 md:mx-0 gap-x-2">
              <div
        ...
        <div className="grid self-stretch grid-cols-1 gap-2 sm:grid-cols-2 md:grid-rows-2 md:gap-4 lg:grid-cols-4-auto lg:grid-rows-1 gap-x-4">
