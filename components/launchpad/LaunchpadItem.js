                        ...
                        <div className="w-[43%] max-[800px]:w-[48%] text-xs">
                            ...
                            <div className="flex justify-between border-b border-[#999999]  mt-1">
                                ...
                                <span className="flex items-center">
                                    {launchpadItem.network !== null && <img className="w-[12px] h-[12px] mr-1" src={addressIcon[launchpadItem.network] && addressIcon[launchpadItem.network]["0x0000000000000000000000000000000000000000"]?.src} alt=""/>}
                                    <span className="font-bold">{launchpadItem.publicPrice === null ? 'na':parseInt(launchpadItem.publicPrice) === 0 ?"Free Mint": launchpadItem.publicPrice/1e18}</span>
                                </span>
                            </div>
                            <div className="flex justify-between border-b border-[#999999] mt-1">
                                <span>Blockchain</span>
                                <span className="font-bold">{chainIdToInfo[parseInt(launchpadItem.network, 16)]?.networkName === 'MANTA'? 'Manta':chainIdToInfo[parseInt(launchpadItem.network, 16)]?.networkName === 'EOS'?'EOS EVM':chainIdToInfo[parseInt(launchpadItem.network, 16)]?.networkName === 'MATIC' ? 'Polygon':chainIdToInfo[parseInt(launchpadItem.network, 16)]?.networkName}</span>
                            </div>
