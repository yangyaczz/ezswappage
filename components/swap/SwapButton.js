...
const SwapButton = ({swapType,formikData, owner,addSwapSuccessCount}) => {
    ...
    const {data, isError, isLoading: waitTrxLoading} = useWaitForTransaction({
        hash: swapType === 'sell'?robustSwapNFTsForTokenData?.hash:robustSwapETHForSpecificNFTsData=== undefined?null : robustSwapETHForSpecificNFTsData.hash,
        confirmations: 1,
        onSuccess(data) {
    ...
    const buttonText = () => {
        ...
        if (swapType === 'sell') {
            ...
            return (
                <div>
                    <button onClick={() => swapNFTToToken()}>{isLoading || waitTrxLoading ? <span class="loading loading-spinner loading-sm"></span> : text}</button>
                </div>
            )
            ...
            return (
                <div>
                    <button onClick={() => swapETHToNFT()}>{swapETHToNFTIsLoading || waitTrxLoading ? <span class="loading loading-spinner loading-sm"></span> : text}</button>
                </div>
            )
        } else {
            return null
