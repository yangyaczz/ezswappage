...
const multiSetFilterPairMode = (swapType, formikData, filteredData, owner, token, setFilterPairs, setSwapMode) => {
    ...
    if (swapType === 'buy') {
        // filter 0 nft pair
        filteredData = filteredData.filter(item => item.nftIds.filter(str => str.length > 0).length !== 0)
        // rebuild pair info
        filteredData = filteredData.map(pair => {
            ...
            let res
            let params = [pair.spotPrice / dec, pair.delta / dec, pair.fee / dec, protocolFee / dec, 1]
            ...
            return {
                ...pair,
                tokenBalance: pair.ethBalance === null ? pair.tokenBalance : pair.ethBalance,   // this pool token balance, vaild or not
