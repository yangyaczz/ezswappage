  ...
  return (
    ...
    <PopupBlurBackground>
      <div className="grid grid-cols-1 grid-rows-[2fr,4fr,2fr,2fr,9fr,1fr,1fr] gap-y-1 justify-items-center content-center items-center w-full h-full text-sm md:text-base lg:text-lg text-white">
        ...
        <PopupHeader
          handlePriceClick={(price) => setBidPrice(price)}
          styleClass=" px-4 py-2 border border-white border-solid w-5/6 max-w-[400px]"
        />
        <section
        ...
        <ConstantLadderSelection id="place_bid_constantladder" popupType="placebids"
          styleClass="grid grid-cols-1 grid-rows-auto self-start gap-2 px-4 py-2 border border-white border-solid w-5/6 max-w-[400px]  rounded-md"/>
        ...
        <button id="place_bid_confirm_button" className={`w-32 ezBtn ezBtnPrimary btn-md`} onClick={() => createBuyPool()}>
          {waitTrxLoading || createPair1155ETHLoading || isLoading ? <span class="loading loading-spinner loading-sm"></span> : "Confirm Bid"}
        </button>
      </div>
    </PopupBlurBackground>
      </div>
